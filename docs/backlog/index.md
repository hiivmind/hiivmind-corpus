...
|---|------|----------|--------|--------|------------|-----------|
| 01 | [Headless enrichment: close the stale-entry loop](01-headless-enrichment.md) | **P1** | In progress | M | — | Re-scan, re-summarize, concept-map, verify, and re-embed stale entries headlessly before the PR is opened. Fixes the 148-stale-entry decay seen in lancedb. |
| 02 | [Result contract as file + shared index-update pattern](02-result-contract-and-shared-update-pattern.md) | **P2** | In progress | S | — | Write `refresh-result.yaml` instead of parsing prose; extract the duplicated refresh algorithm into one pattern doc; surface `skipped-manual` local sources. |
| 04 | [Python packaging (PEP 723 + uv), cache-path bug, CI](04-python-packaging-and-ci.md) | **P2** | In progress | S | — | Self-contained `uv run` scripts, fix `FASTEMBED_CACHE_PATH` bug in detect.py, dedupe constants, add CI to the plugin repo. |
